 ...
 */
internal class SheetFaces(val load: suspend (String) -> ImageBitmap?) {
    val cache = mutableStateMapOf<String, ImageBitmap?>()
}

internal val LocalSheetFaces = staticCompositionLocalOf { SheetFaces { null } }

@Composable
internal fun ProvideSheetFaces(load: suspend (String) -> ImageBitmap?, content: @Composable () -> Unit) {
    val faces = remember(load) { SheetFaces(load) }
    CompositionLocalProvider(LocalSheetFaces provides faces, content = content)
}
...
internal fun Face(userId: String?, name: String, size: Dp, modifier: Modifier = Modifier) {
    val faces = LocalSheetFaces.current
