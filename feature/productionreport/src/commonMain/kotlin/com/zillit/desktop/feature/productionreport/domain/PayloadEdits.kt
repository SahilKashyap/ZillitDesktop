...
/**
 * Every structural edit of a report document, as pure functions.
 *
 ...
 * copied: orders are renumbered on every change, positions are clamped, and a
...
sealed interface EditorSelection {
    /** The report header ("Production Report Info"). */
...
/** The header block's slot: "before page row N", clamped into the document. */
...
/**
 * `addPageRowAfter`: a new row (or page break) at `afterIndex + 1`, the header
 * and approvers blocks shifted so they stay where the user sees them.
...
/** A whole page row removed, with the header and approvers blocks kept in place. */
 ...
 * it into whichever row had taken the index — a lone Address came back
 * squeezed beside Schedule Summary.
 */
...
/**
 * The Type select (`section` / `table` / `notes`), per the web's conversion
 * rules: notes keeps one text column and one blank note; a section keeps the
 * first two columns of every line; a table keeps everything when it already
 * has two columns. Crew and weather cells never change type.
 */
fun PageCell.convertedTo(kind: CellKind): PageCell {
    if (kind == this.kind || renderAs != RenderKind.Generic) return this
    return when (kind) {
        CellKind.Notes -> copy(
            kind = kind,
            rawKind = kind.wire,
        ...
        CellKind.Section -> copy(
            kind = kind,
            rawKind = kind.wire,
            columns = if (columns.size >= 2) columns.take(2) else FIELD_VALUE_COLUMNS,
            rows = rows.ifEmpty { listOf(CellRow(0)) }.mapIndexed { index, line ->
                CellRow(
                    order = index,
                    values = List(2) { line.values.getOrNull(it) ?: CellValue() },
                    extras = line.extras,
                )
            },
        )
        CellKind.Table -> if (columns.size >= 2) {
            ...
            copy(
                ...
                rows = rows.ifEmpty { listOf(CellRow(0)) }.mapIndexed { index, line ->
                    CellRow(order = index, values = listOf(line.values.firstOrNull() ?: CellValue(), CellValue()))
                },
            )
        }
        CellKind.Unknown -> this
    }
}

private val FIELD_VALUE_COLUMNS = listOf(ColumnSpec(label = "Field"), ColumnSpec(label = "Value"))

/** A blank column appended, every line padded to match. */
 ...
 */
fun SheetPayload.withDate(dateYmd: String): SheetPayload {
    if (dateYmd == shared.dateYmd) return this
    ...
    return copy(shared = shared.copy(dateYmd = dateYmd), rows = cleared)
}
...
/** Approver toggled in the payload's own list — appended, or removed. */
fun SheetPayload.toggledApprover(userId: String): SheetPayload {
    ...
    return copy(shared = shared.copy(approverIds = next))
}
